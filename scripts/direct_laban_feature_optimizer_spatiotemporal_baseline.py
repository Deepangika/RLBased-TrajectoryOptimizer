...
def optimise(args, external_target_profile: Dict[str, float] | None = None):
    ...
    def objective(x: np.ndarray) -> float:
        ...
        feature_loss, feature_diag = weighted_feature_rmse(var_norm, target_profile, feature_weights)
        
        # Add per-dimension penalties to constrain features to target values
        # Each penalty is: weight * |value - target|, clamped to [0, 1] range
        weight_deviation = abs(float(var_norm.get("weight", 0.0)) - float(target_profile["weight"]))
        weight_target_penalty = max(0.0, float(args.weight_target_weight) * float(np.clip(weight_deviation, 0.0, 1.0)))
        
        flow_boundness_deviation = abs(float(var_norm.get("flow_boundness", 0.0)) - float(target_profile["flow_boundness"]))
        flow_boundness_target_penalty = max(0.0, float(args.flow_boundness_target_weight) * float(np.clip(flow_boundness_deviation, 0.0, 1.0)))
        
        space_indirectness_deviation = abs(float(var_norm.get("space_indirectness", 0.0)) - float(target_profile["space_indirectness"]))
        space_indirectness_target_penalty = max(0.0, float(args.space_indirectness_target_weight) * float(np.clip(space_indirectness_deviation, 0.0, 1.0)))
        
        shape_arcness_deviation = abs(float(var_norm.get("shape_arcness", 0.0)) - float(target_profile["shape_arcness"]))
        shape_arcness_target_penalty = max(0.0, float(args.shape_arcness_target_weight) * float(np.clip(shape_arcness_deviation, 0.0, 1.0)))
        
        time_deviation = abs(float(var_norm.get("time", 0.0)) - float(target_profile["time"]))
        time_target_penalty = max(0.0, float(args.time_target_weight) * float(np.clip(time_deviation, 0.0, 1.0)))
        ...
        total_loss = (
            feature_loss
            + args.preserve_weight * joint_preservation_error
            + args.nearest_path_weight * terms["nearest_path_mse"]
            + args.endpoint_weight * terms["endpoint_error"]
            + args.direction_weight * terms["direction_error"]
            + args.path_length_weight * terms["path_length_error"]
            + args.detour_weight * terms["detour_error"]
            + args.max_dev_weight * terms["max_dev_error"]
            + args.smooth_weight * smoothness_error
            + args.joint_limit_weight * joint_limit_error
            + args.coeff_weight * spatial_coeff_error
            + args.time_coeff_weight * timing_coeff_error
            + args.time_warp_weight * terms["time_warp_deviation"]
            + args.time_roughness_weight * terms["time_roughness"]
            + weight_target_penalty
            + flow_boundness_target_penalty
            + space_indirectness_target_penalty
            + shape_arcness_target_penalty
            + time_target_penalty
        )
        if eval_count % 25 == 0:
            row = {
                ...
                "weight_target_penalty": float(weight_target_penalty),
                "flow_boundness_target_penalty": float(flow_boundness_target_penalty),
                "space_indirectness_target_penalty": float(space_indirectness_target_penalty),
                "shape_arcness_target_penalty": float(shape_arcness_target_penalty),
                "time_target_penalty": float(time_target_penalty),
            ...
            print(
                ...
                f"W={var_norm.get('weight', np.nan):>5.3f}, T={var_norm.get('time', np.nan):>5.3f}, "
                f"F={var_norm.get('flow_boundness', np.nan):>5.3f}, S={var_norm.get('space_indirectness', np.nan):>5.3f}, "
                f"Sh={var_norm.get('shape_arcness', np.nan):>5.3f}"
            )
    ...
    print_section("POLISHING WITH L-BFGS-B")
    result_local = minimize(result_de.x if False else objective, x0=result_de.x, method="L-BFGS-B", bounds=bounds, options={"maxiter": args.local_maxiter, "ftol": 1e-9})
